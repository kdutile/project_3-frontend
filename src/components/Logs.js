...
const Logs = (props) => {
    ...
    const [displayLogs, setDisplayLogs] = useState(
        props.allLogs.filter((log) => props.currentUser.username === log.user)
    );
    ...
    const showAllToggler = () => {
        ...
        if (showAll) {
            setShowAll(false);
            setDisplayLogs(
                props.allLogs.filter(
                    (log) => props.currentUser.username === log.user
                )
            );
        }
    ...
    return (
        <section className="logsBackground">
            {showAll ? (
                <div className="buttons has-addons is-centered section">
                    <button className="button" onClick={showAllToggler}>
                        Only Mine
                    </button>
